...
void adjust_icon_state(DMI *icon, icon_state* iconState){
    ...
    png_uint_32 start_row = ((icon->frame_count * icon->icon_row_bytes) / icon->image->row_bytes)* icon->icon_height;
    for(int i = 0; i < iconState->dirs; i++){
...
/* This is meant to be a function that takes a DMI as an argument and creates a new DMI*/
void export_as_dmi(DMI* dmi, char** list_of_icon_states){
